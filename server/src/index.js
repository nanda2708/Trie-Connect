...
// Contact book: MongoDB stores complete records; the C++ Trie indexes both names and phone numbers.
app.get("/api/contacts", async (req, res) => {
  try {
    ...
    if (/^\d/.test(query)) {
      ...
      const phones = (trieMatches.words || []).filter(value => value.startsWith(PHONE_KEY)).map(value => value.slice(PHONE_KEY.length));
    ...
    const names = (trieMatches.words || []).filter(value => value.startsWith(NAME_KEY)).map(value => value.slice(NAME_KEY.length));
...
async function hydrateTrie() {
  ...
  for await (const document of cursor) {
    ...
    const phone = String(document.phone || "").replace(/\D/g, "");
