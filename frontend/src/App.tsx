...
export default function App() {
  ...
  async function search(value: string) {
    setPrefix(value);
    if (!value.trim()) { setResults([]); setCount(0); return; }
    setLoading(true);
    try {
  ...
  async function addWord() {
    ...
    try {
      ...
      await refreshStats();
      if (prefix) await search(prefix);
    } catch (error) { setMessage(error instanceof Error ? error.message : "Insert failed"); }
  }
    ...
    <main className="min-h-screen bg-[#f7f8f6] text-[#17231f]">
      <header className="border-b border-[#dfe6e1] bg-white/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 md:px-8">
          <div className="flex items-center gap-3"><div className="grid h-9 w-9 place-items-center rounded-lg bg-[#194d3c] text-white"><GitBranch size={18} /></div><div><div className="font-semibold tracking-tight">TrieConnect</div><div className="text-[10px] uppercase tracking-[.18em] text-[#789087]">Prefix search engine</div></div></div>
      ...
      <section className="grid-bg border-b border-[#dfe6e1]">
        <div className="mx-auto max-w-6xl px-5 py-16 md:px-8 md:py-24">
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-xs text-[#6d7c75]"><span className="rounded-full border border-[#d8e2dc] bg-white px-3 py-1.5">O(L) prefix lookup</span><span className="rounded-full border border-[#d8e2dc] bg-white px-3 py-1.5">C++17</span><span className="rounded-full border border-[#d8e2dc] bg-white px-3 py-1.5">Express API</span><span className="rounded-full border border-[#d8e2dc] bg-white px-3 py-1.5">MongoDB ready</span></div>
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-6xl gap-5 px-5 py-10 md:grid-cols-3 md:px-8">
        ...
        <Metric icon={<Database size={18} />} label="Persistence" value="MongoDB" detail="Optional; configured through MONGODB_URI" />
      </section>

      <section className="mx-auto grid max-w-6xl gap-6 px-5 pb-20 md:grid-cols-[1fr_360px] md:px-8">
        <div className="rounded-2xl border border-[#dfe6e1] bg-white p-6"><div className="flex items-start justify-between gap-4"><div><h2 className="font-semibold">Work with the data structure</h2><p className="mt-1 text-sm text-[#74817b]">Insert a word and watch the node count change.</p></div><button onClick={loadSample} className="flex items-center gap-2 rounded-lg bg-[#194d3c] px-3 py-2 text-xs font-semibold text-white hover:bg-[#28634f]"><Upload size={14} /> Load sample</button></div><div className="mt-6 flex gap-2"><input value={word} onChange={e => setWord(e.target.value)} onKeyDown={e => e.key === "Enter" && addWord()} placeholder="Add a word" className="h-10 flex-1 rounded-lg border border-[#d7e0db] px-3 text-sm outline-none focus:border-[#39745e]" /><button onClick={addWord} className="rounded-lg border border-[#194d3c] px-4 text-sm font-semibold text-[#194d3c] hover:bg-[#edf5f0]">Insert</button></div></div>
        <div className="rounded-2xl border border-[#dfe6e1] bg-[#193f34] p-6 text-white"><div className="flex items-center gap-2 text-[#b9dcca]"><GitBranch size={17} /><span className="text-xs font-bold uppercase tracking-[.15em]">How it works</span></div><div className="mt-5 space-y-4 text-sm leading-6 text-[#d2dfd9]"><p><b className="text-white">1.</b> Express receives the prefix.</p><p><b className="text-white">2.</b> The C++ engine walks one character at a time.</p><p><b className="text-white">3.</b> The subtree is traversed to collect suggestions.</p><p><b className="text-white">4.</b> MongoDB stores words when persistence is configured.</p></div><div className="mt-6 flex items-center gap-2 text-xs text-[#9fc0b0]">{message}<ArrowRight size={13} /></div></div>
      </section>
    </main>
  );
}
